...
def generate(
    ...
    input_gs_variables = []
    if input_variables:
        # This is changed to inject constants, instead of variables
        for key, value in input_variables.items():
            if (key.endswith('_const')):
                v = gs.Constant(name=key, values=np.array(value[1]).astype(value[0]))
                input_gs_variables.append(v)
            else:
                v = gs.Variable(name=key, dtype=value[0], shape=value[1])
                input_gs_variables.append(v)
